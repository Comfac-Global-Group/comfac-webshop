...
class WebstoreItemGroup(ItemGroup, WebsiteGenerator):
	...
	def validate(self):
		...
		super(WebstoreItemGroup, self).validate()

	def on_update(self):
		invalidate_cache_for(self)
		super(WebstoreItemGroup, self).on_update()
	...
	def on_trash(self):
		...
		super(WebstoreItemGroup, self).on_trash()

	def get_context(self, context):
		context.show_search = True
		context.body_class = "product-page"
		context.page_length = (
			cint(frappe.db.get_single_value("Webstore Settings", "products_per_page")) or 6
		)
